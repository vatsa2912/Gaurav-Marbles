        ...
        <div className="space-y-4">
          ...
          <div className="block md:hidden space-y-3">
            {filteredInvoices.map((inv) => {
              ...
              const customerName = inv.consignee?.name || inv.buyer?.name || "Walk-in Customer";
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                    <div>
                      <span className="text-[10px] text-slate-400 uppercase font-semibold block">Grand Total</span>
                      <span className="font-bold text-slate-900 text-sm">
                        ₹{Number(inv.grandTotal || 0).toLocaleString("en-IN", { maximumFractionDigits: 2 })}
                      </span>
                    </div>
                    <div>
                    ...
                    <div className="col-span-2 pt-1.5 border-t border-slate-200/60 flex items-center justify-between text-xs text-slate-600">
                      <span>Items: {(inv.items || []).length}</span>
                      <span>Taxable: ₹{Number(inv.subtotal || 0).toLocaleString("en-IN", { maximumFractionDigits: 0 })}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-1 border-t border-slate-100">
                    ...
                    <Link
                      href={`/dashboard/invoices/${inv.id}`}
                      className="flex-1 py-2.5 px-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs transition inline-flex items-center justify-center gap-1.5 min-h-[44px]"
                    >
