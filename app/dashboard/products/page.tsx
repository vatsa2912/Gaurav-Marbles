                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-slate-500 mt-0.5 flex flex-wrap items-center gap-1.5">
                        ...
                        {isTiles && product.piecesPerBox && <span>• {product.piecesPerBox} pcs/box</span>}
                      </div>
                    </div>
                    <div className="shrink-0">{getStockBadge(product)}</div>
                  </div>

                  <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                    <div>
                      <span className="text-[10px] text-slate-400 uppercase font-semibold block">Selling Rate</span>
                      <span className="font-bold text-slate-900 text-sm">
                        ₹{Number(product.sellingPrice).toLocaleString("en-IN")}
                      </span>
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-400 uppercase font-semibold block">Physical Stock</span>
                      <span className="font-bold text-slate-900 text-sm">
                    ...
                    {showEstimatedStockColumn && (
                      <div className="col-span-2 pt-1.5 border-t border-slate-200/60 flex items-center justify-between text-xs">
                        <span className="text-[10px] text-slate-500 uppercase font-medium">Estimated Stock</span>
                        <span className="font-semibold text-slate-800">{renderEstimatedStock(product)}</span>
                      </div>
                    )}
                  </div>
                  ...
                  {lots.length > 0 && (
                    <div className="text-[11px] text-slate-500 flex items-center justify-between pt-1 border-t border-slate-100">
                      <span className="text-slate-400">Lot Breakdown:</span>
